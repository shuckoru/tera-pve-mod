...
Personal dps: ${personalDps}`,
  ...
  TrackedAbnormalityActivated: (icon) =>
    `<img src="img://__${icon}"width="48"height="48"vspace="-20"/><font size="24">&nbsp;activated</font>`,
  ...
  PartyDPS: (event) =>
    `${(
      Number(event.maxHp - event.curHp) /
      1000000 /
...
const abnormsHelpersMod = (mod) => {
  ...
  mod.hook("S_ABNORMALITY_BEGIN", "*", (event) => {
    ...
    if (event.id in modConfigs.trackedAbnormalities.data) {
      const { dpsMode, tankMode, icon, tankDebuff, supportDebuff } =
...
const dpsMeterMod = (mod) => {
  ...
  mod.hook("S_BOSS_GAGE_INFO", "*", (event) => {
    ...
    inFight: if (event.curHp < event.maxHp) {
      if (fightStartInSeconds == 0) {
        ...
        personalDamageDone = 0;
        sendCustomStyleMessage(mod)(
          Messages.FightStarted,
          ...Object.values(MessageTypes.FightStarted)
        );
      }
      ...
      if (switches.burnMsgEnabled)
        if (
          fightDurationInSeconds % 60 != 0 &&
          (fightDurationInSeconds % 60) % 58 == 0
        )
          sendCustomStyleMessage(mod)(
            Messages.BurnIn2,
            ...Object.values(MessageTypes.BurnIn2)
          );
    }
  });
  ...
  mod.hook("S_DESPAWN_NPC", "*", (event) => {
    ...
    sendCustomStyleMessage(mod)(
      lastFightDetailsMsg,
      ...Object.values(MessageTypes.EndOfFight)
    );
    if (switches.autoPasteFightDetailsInPartyChat)
...
const skillsDamageValueTester = (mod) => {
  mod.hook("S_EACH_SKILL_RESULT", "*", { order: 100 }, (event) => {
    ...
    if (event.value > 0n) {
      if (
        ...
        damageValueTesterSkillsCast[event.skill.id].eventId != event.id
      )
        delete damageValueTesterSkillsCast[event.skill.id];

      let eventTime =
        firstTickSkills[event.skill.id] &&
        damageValueTesterSkillsCast[event.skill.id]
          ? damageValueTesterSkillsCast[event.skill.id].time
          : event.time / 1000;
      let dmg = damageValueTesterSkillsCast[event.skill.id]
        ? Number(damageValueTesterSkillsCast[event.skill.id].damage) +
          Number(event.value)
        : Number(event.value);
      ...
      damageValueTesterSkillsCast[event.skill.id] = {
        ...
        time: eventTime,
        damage: dmg,
        damageValue: (dmg / 10000000 / eventTime).toFixed(3),
        timestamp: Date.now(),
      };

      sendInGameCmdMessage(mod)(
        switches.dmgValueTestVerboose
          ? Messages.DamageValueVerbose(event, eventTime, dmg)
          : Messages.DamageValueRaw(event, eventTime, dmg)
      );
    }
  });
};
